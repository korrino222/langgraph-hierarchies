---
title: SubchainPolicy
sidebarTitle: SubchainPolicy
description: Declarative per-subchain state isolation — clear, merge, and discard fields across hierarchy boundaries.
---
...
`SubchainPolicy` declares what happens at each subchain **entry** and **exit**:
...
| --- | --- | --- |
| `clear_messages` | `True` | Strip inherited message history on subchain entry |
...
sequenceDiagram
    participant Parent
    participant Policy as SubchainPolicy
    participant Child
...
See [`examples/irs_reporting/`](https://github.com/korrino222/langgraph-hierarchies/tree/main/examples/irs_reporting) for a five-stage pipeline demonstrating this pattern. Run it with:
